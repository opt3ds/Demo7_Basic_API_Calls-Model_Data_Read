 ...
 *
 * Usage:
 * 1. Specify the port via --port=18084: node server.js --port=18084
 * 2. Specify the port via -p 18084: node server.js -p 18084
 * 3. Specify the port via --port 18084: node server.js --port 18084
 *
...
const OUTPUT_MODEL_PATH = path.join(__dirname, '../VuePage/public/static/OutputModel');
...
process.on('unhandledRejection', (reason, promise) => {
    ...
    // Do not exit the process, only log the error
});
...
// Configure the path of the Vue build output (modify according to the actual path)
const vueDistPath = path.join(__dirname, '../VuePage/dist');
const vuePublicPath = path.join(__dirname, '../VuePage/public');

// Mount the public directory (in Vite projects, the public directory content is mapped to the root path)
...
if (fs.existsSync(vueDistPath)) {
    ...
    console.warn(`Warning: Vue build directory does not exist: ${vueDistPath}`);
    console.warn('Please make sure the Vue project has been built and placed at the specified location');
}
...
app.get('/api/app/model/GetFloorStructureData', async (req, res) => {
    let db = null;
    try {
        // Get values from query parameters instead of path parameters
        const { lightweightName, pid} = req.query;

        // Check required parameters
        if (!lightweightName) {
            return res.status(400).json({code: 0, codeMsg: 'lightweightName parameter is required', datas: []});
        }

        const dbPath = path.join(OUTPUT_MODEL_PATH, lightweightName, `${lightweightName}.db`);

        // Check whether the database file exists
        if (!fs.existsSync(dbPath)) {
            return res.status(404).json({code: 0, codeMsg: `Database ${lightweightName} does not exist`, datas: []});
        }

        db = await connectToDatabase(dbPath);
        const tables = await executeQuery(db, "SELECT * FROM sqlite_master where tbl_name='model_tree'");
        if (tables.length === 0) {
            return res.status(404).json({
                code: 0,
                codeMsg: `Table model_tree does not exist in database ${lightweightName}`,
                datas: []
            });
        } else {
            // If pid is empty or undefined, default to '0'
            const parentId = (pid == '' || pid == null || pid == undefined) ? '0' : pid;
            const floorStructureData = await executeQuery(db, "SELECT * FROM model_tree where pGlid=?", [parentId]);
...
app.get('/api/app/model/GetProfessionalStructureData', async (req, res) => {
    ...
    try {
        ...
        db = await connectToDatabase(dbPath);
        const tables = await executeQuery(db, "SELECT * FROM sqlite_master where tbl_name='model_type'");
        if (tables.length === 0) {
            return res.status(404).json({
                code: 0,
                codeMsg: `Table model_type does not exist in database ${lightweightName}`,
                datas: []
            });
        } else {
            // If pid is empty or undefined, default to '0'
            const parentId = (pid == '' || pid == null || pid == undefined) ? '0' : pid;
            const floorStructureData = await executeQuery(db, "SELECT * FROM model_type where pGlid=?", [parentId]);
            if (floorStructureData.length === 0) {
                res.json({code: 1, datas: []});
            } else {
                res.json({code: 1, datas: floorStructureData});
...
app.get('/api/app/model/GetPropertyDataByExternalId', async (req, res) => {
    ...
    try {
        ...
        db = await connectToDatabase(dbPath);
        const tables = await executeQuery(db, "SELECT * FROM sqlite_master where tbl_name='model_property'");
        if (tables.length === 0) {
            return res.status(404).json({
                code: 0,
                codeMsg: `Table model_property does not exist in database ${lightweightName}`,
                datas: []
            });
        } else {
            const sql = `select *
...
// ========== SPA route support (must be placed after all API routes) ==========
if (fs.existsSync(vueDistPath)) {
    // SPA route support: all non-API requests return index.html
    app.get('*', (req, res) => {
        // If it is an API request, do not handle it (it should have been handled by the routes above)
        if (req.path.startsWith('/api')) {
            return res.status(404).json({code: 0, codeMsg: 'API endpoint not found', datas: []});
        }
...
app.listen(PORT, () => {
    ...
    console.log(`Model database path: ${OUTPUT_MODEL_PATH}`);
    console.log(`Access address: http://${HOST}:${PORT}`);
});
