...
interface Props {
  title?: string; // The dialog title, defaults to "Notice"
  footer?: null; // The dialog footer, defaults to null
  visible: boolean; // Whether the dialog is visible
  confirmLoading?: boolean; // Whether the OK button is in the loading state, defaults to false
  ...
  centered?: boolean; // Whether the dialog is displayed centered, defaults to false
  width?: number;
}
...
// Create the modalTitleRef ref, used to reference the title element in the component
const modalTitleRef = ref<HTMLElement | null | any>(null);

// Use the useDraggable hook to get drag-related properties
const { x, y, isDragging } = useDraggable(modalTitleRef);

// Create the emit function for triggering custom events
const emit = defineEmits(["ok", "update:visible", "cancel"]);

// Function that handles the ok event
const handleOk = (e: MouseEvent) => {
  emit("ok");
};

// Function that handles the cancel event
const handleCancel = () => {
...
// Watch changes of x and y
watch([x, y], () => {
  // Function that listens for mouse move events
  if (!startedDrag.value) {
    ...
    const bodyRect = document.body.getBoundingClientRect(); // Get the bounding info of the page body element
    const titleRect = modalTitleRef.value.getBoundingClientRect(); // Get the bounding info of the modalTitle element
  ...
  startedDrag.value = true; // Set the dragging started flag
});

// Watch changes of isDragging
watch(isDragging, () => {
  // Listen for changes of the isDragging variable
  if (!isDragging) {
...
// Use watchEffect to listen for changes of reactive data
watchEffect(() => {
  // Respond to changes of startedDrag.value
  if (startedDrag.value) {
    // If startedDrag.value is true, a drag operation is in progress
    transformX.value = // Calculate the x offset
      preTransformX.value +
      Math.min(Math.max(dragRect.value.left, x.value), dragRect.value.right) -
      startX.value;
    transformY.value = // Calculate the y offset
      preTransformY.value +
...
// Compute transformStyle, used to dynamically set the dialog position
