...
const attributeInformation = ref("Please click to pick a feature");
...
function onTabChange(activeKey: string) {
  if (activeKey === "properties") {
    if (attributeList.value[0] && attributeList.value[0].content.length > 0) {
      ...
      attributeInformation.value = "No data yet~";
    }
  } else {
...
async function fetchProperties(featureId: string) {
  ...
  attributeInformation.value = "Querying, please wait~";
  hasData.value = false;
  if (!featureId) {
    attributeInformation.value = "Unable to get the feature external ID";
    return;
  }
  ...
  try {
    ...
    attributeInformation.value = "Failed to get properties";
  }
}
